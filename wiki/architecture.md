...
| `progress-recap.tsx` (server) | RSC streamed via `<Suspense>` near the top of the home page (above suggestions). Calls `getProgressRecap()` → mastery histogram (segmented meter), a hand-rolled inline-SVG two-line curve (cards seen vs mastered over ~12 weeks), and a `learn`/`consolidate` phase badge. Returns `null` when the user has no cards. See [[fsrs#home-page-progress-recap]]. |
| `progress-recap-skeleton.tsx` (server) | Pulse placeholder rendered as Suspense fallback (badge + meter + curve blocks). |
| `suggested-themes.tsx` (server) | RSC streamed via `<Suspense>` at the bottom of the home page. Calls `getSuggestedThemes()` → 6 broad general-culture category themes (generalized from the user's recent study profile), or a curated fallback list if profile < 10 cards. Each theme links to `/explore?theme=<label>`. |
