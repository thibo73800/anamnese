...
| Install works on desktop but Chrome Android shows no prompt (HTTPS prod, SW registered) | The manifest only declares a 512×512 PNG — Chrome Android strictly requires an additional **≥192×192** icon | Verify `app/manifest.ts` declares entries for `/icon-192` (192), `/icon` (512), and `/icon-maskable` (512, `purpose: 'maskable'`). Clear Chrome Android site data to force re-evaluation — it caches the "not installable" verdict. |
